...
(function () {
  "use strict";
  !(function (e, t) {
    "object" == typeof exports && "undefined" != typeof module ? t(require("lodash")) : "function" == typeof define && define.amd ? define(["lodash"], t) : t((e = e || self)._);
  })(this, function (e) {
    "use strict";
    (e = e && Object.prototype.hasOwnProperty.call(e, "default") ? e.default : e).mixin({
  ...
  function futweb() {
    ...
    info = {
      ...
      setfield: {
        card: ["pos", "price", "other", "club", "low", "meta"],
        player: ["auction", "futbin", "getprice", "loas", "uatoclub", "transfertoclub", "pickbest"],
        sbc: ["top", "right", "quick", "duplicate", "records", "input", "icount", "template", "templatemode", "market", "sback", "cback", "dupfill", "autofill", "squadcmpl", "conceptbuy", "meetsreq", "headentrance"],
        info: ["obj", "sbc", "sbcf", "sbcs", "pack", "squad", "skipanimation", "sbcagain", "packagain"],
      },
      set: {},
      lock: [],
    ...
    cntlr = {
      //getAppMain().getRootViewController().getPresentedViewController().getCurrentViewController().getCurrentController();
      current: function () {
    ...
    events.init = async function () {
      ...
      let history_a = JSON.parse(GM_getValue("history", "[]")),
        history_b = [];
                ...
                GM_xmlhttpRequest({
                  ...
                  onload: function (res) {
                    ...
                    if (_.has(metaJson, "bodyType")) {
                      info.meta.bodyType = _.fromPairs(_.flatMap(metaJson.bodyType, (ids, bodyType) => ids.map((id) => [id, Number(bodyType)])));
                    }
                ...
                if (tt.success && tt.data && !JSUtils.isString(tt.data)) {
                  ...
                  if (cntlr.current().className == "UTHomeHubViewController" && info.evolutions.newCount > 0) {
                    ...
                    cntlr
                      .current()
                      .getView()
                      ._academyTile.__tileContent.before(events.createDF(`<div class="fsu-task">${info.evolutions.html}</div>`));
                  }
                }
              });
            });
    ...
    UTHomeHubView.prototype.getAcademyTile = function () {
      if (info.evolutions.newCount > 0 && !this._academyTile.__root.querySelector(".fsu-task")) {
        this._academyTile.__tileContent.before(events.createDF(`<div class="fsu-task">${info.evolutions.html}</div>`));
      }
    ...
    events.getItemBy = (type, queryOptions, insertData, replaceData) => {
      ...
      if (!("unlimited" in queryOptions) || ("unlimited" in queryOptions && !queryOptions.unlimited)) {
        players = players.filter((i) => {
          return i.isPlayer() && i.loans === -1 && !i.isEnrolledInAcademy() && i.endTime == -1;
        });
      } else {
        players = players.filter((i) => {
          return i.isPlayer();
      ...
      const isMixBronzeAndSilver = _.isEqual(queryOptions.rareflag, [0, 1]) && (queryOptions.rs == 0 || queryOptions.rs == 1);

      const sortField = isMixBronzeAndSilver
        ? ["rareflag", "rating", sortItemPresence, "untradeableCount", "_itemPriceLimits.minimum", "_itemPriceLimits.maximum"]
        : ["rating", sortItemPresence, "untradeableCount", "rareflag", "_itemPriceLimits.minimum", "_itemPriceLimits.maximum"];

      const sortOrder = isMixBronzeAndSilver ? ["asc", ratingOrder, "asc", "desc", "asc", "asc"] : [ratingOrder, "asc", "desc", "asc", "asc", "asc"];

      players = _.orderBy(players, sortField, sortOrder);

      if (specialOrder.length && players.length) {
        if (_.includes(specialOrder, 1)) {
          let tempPlayers = _.values(_.groupBy(players, "rating")),
            resultPlayers = [];
          if (ratingOrder == "desc") {
      ...
      if (insertData && !replaceData) {
        ...
        let insertPlayerIds = _.map(
          _.filter(insertData, (i) => {
    ...
    info.localization = {
      ...
      "notice.marketsetmax": [
        "已修改优化搜索信息可直接搜索 如无结果请返回调整参数",
        "已修改優化搜尋資訊，可直接搜尋。如無結果，請返回調整參數。",
        "Optimizations have been made to the search information. You can now search directly. If there are no results, please return and adjust the parameters.",
      ],
      ...
      "loas.popupm": [
        "已选择本列表中 %1 个球员拍卖价格大致为 %2 ，请点击确认开始陆续上架拍卖，途中可点击加载图标下文字取消。",
        "已選擇本列表中 %1 個球員拍賣價格大致為 %2 ，請點擊確認開始陸續上架拍賣，途中可點擊加載圖標下文字取消。",
        "You have selected approximately %1 players from this list, with an estimated auction price of %2. Please click confirm to start listing them for auction one by one. You can click the text below the loading icon to cancel during the process.",
      ],
      ...
      "notice.phoneloas": [
        "请注意手机端挂牌后需重新进入拍卖清单才会刷新显示。",
        "請注意手機端掛牌後需重新進入拍賣清單才會重新整理顯示。",
        "Please note that after listing on the mobile terminal, you need to re-enter the auction list before refreshing the display.",
      ],
      "notice.builder": [
        "通过排除后球员数量已不足以填充阵容，如需要请调整条件再次搜索。",
        "通過排除後球員數量已不足以填充陣容，如需要請調整條件再次搜尋。",
        "The number of players after exclusion is no longer sufficient to fill the lineup, please adjust the criteria to search again if necessary.",
      ],
      "notice.conceptdiff": [
        "发现所购买的假想球员有多个版本，已经将非搜索版本的亮度。",
        "發現所購買的假想球員有多個版本，已經將非搜尋版本的亮度。",
        "Found that there are multiple versions of the purchased hypothetical player, the brightness of the non-searched version has been added.",
      ],
      ...
      "squadback.popupm": [
        "请注意，阵容回退后将无法再返回到此阵容，还可回退 %1 次。",
        "請注意，陣容回退後將無法再返回到此陣容，還可回退 %1 次。",
        "Note that the squad will no longer be able to return to this lineup after retreating, and can go back %1 times.",
      ],
      "sbc.squadback": ["退", "退", "B"],
      ...
      "swaptradable.popupm": [
        "点击确定可将未分配中球员与队内可交易球员交换，共可交换 %1 个。",
        "点击确定可将未分配中球员与队内可交易球员交换，共可交换 %1 个。",
        "Click OK to exchange unassigned players with tradable players in the team, for a total of %1 players.",
      ],
      ...
      "notice.ldataerror": [
        "球员数据加载失败 请重刷新页面加载 否则核心功能无法使用",
        "球員資料載入失敗 請重重新整理頁面載入 否則核心功能無法使用",
        "Player data loading failed, please refresh the page to load, otherwise the core functions cannot be used",
      ],
      ...
      "numberofqueries.popupm": [
        "此处影响在购买球员的查询次数，初次使用futbin读取价格，其后每次按照搜索出结果进行下次查询价格，查询价格变化按照拍卖价格+、-变化，可自行在拍卖输入价格点击+、-后查看，具体规则请阅读说明文档。<br>默认配置为5次，最低可设置为1次，不建议次数过多。",
        "此處影響在購買球員的查詢次數，初次使用futbin讀取價格，其後每次按照搜尋出結果進行下次查詢價格，查詢價格變化按照拍賣價格+、-變化，可自行在拍賣輸入價格點選+、-後檢視，具體規則請閱讀說明文件。<br>預設配置為5次，最低可設定為1次，不建議次數過多。",
        "This affects the number of inquiries in the purchase of players. Use futbin to read the price for the first time, and then check the price for the next time according to the search results. The query price changes according to the auction price + and -. You can enter the price in the auction by yourself and click + and -. Please read the description document for specific rules. < br > The default configuration is 5 times, and the minimum can be set to 1 time. It is not recommended to use too many times.",
      ],
      ...
      "consult.popupm": [
        "支持导入FUTBIN和FUT.GG两个网站的SBC方案ID或网址，为空则默认读取FUTBIN价格最低的5个方案进行计算。",
      ...
      "squadcmpl.popupm": [
        "阵容补全即会将假想球员替换为同评分球员、空位替换为所填评分。请填入评分需要数字，以英文逗号组合，单个评分将会替换所有空位，多个将替换指定个数空位。",
        "陣容補全即會將假想球員替換為同評分球員、空位替換為所填評分。請填入評分需要數字，以英文逗號組合，單個評分將會替換所有空位，多個將替換指定個數空位。",
        "Lineup completion will replace hypothetical players with players of the same rating, and vacancies with the filled rating. Please fill in the numbers required for the rating, combined with English commas, a single rating will replace all vacancies, and multiple will replace the specified number of vacancies.",
      ],
      "squadcmpl.popupmsup": [
        "模拟计算结果可能略有偏差，可点击按钮前往网站进行自由计算。",
        "模擬計算結果可能略有偏差，可點選按鈕前往網站進行自由計算。",
        "The simulation results may be slightly biased, and you can click the button to go to the website for free calculation.",
      ],
      "shieldlea.popupm": [
        "此处为排除的联赛设置（需开启排除联赛按钮才生效），点击右侧可切换状态，开关旁为此联赛球员数。",
        "此處為排除的聯賽設定（需開啟排除聯賽按鈕才生效），點選右側可切換狀態，開關旁為此聯賽球員數。",
        "Here is the excluded league setting (you need to turn on the excluded league button to take effect), click on the right to switch the status, and the number of players in this league is next to the switch.",
      ],
      "popupButtonsText.44401": ["前往网站计算", "前往網站計算", "Go to the website to calculate"],
      "squadcmpl.simulatedsuccess": [
        "此次模拟补全后阵容评分： %1 ，预估填充球员价值： %2 。",
        "此次模擬補全後陣容評分： %1 ，預估填充球員價值： %2 。",
        "Lineup score after this simulation completion: %1 , estimated fill player value: %2 .",
      ],
      "squadcmpl.simulatederror": [
        "无法模拟补全出阵容，请填充球员、调整排除选项或进入网站计算。",
        "無法模擬補全出陣容，請填充球員、調整排除選項或進入網站計算。",
        "The full lineup cannot be simulated. Please fill in players, adjust exclusion options, or enter the website for calculations.",
      ],
      ...
      "squadcmpl.popupmsupallconcept": [
        "此次将尝试替换假想球员，不会考虑挑战要求，如无法替换代表无此评分球员。",
        "此次將嘗試替換概念球員，不會考慮挑戰條件，如無法替換代表無此評分球員。",
        "This time, attempts will be made to replace concept players, without considering challenge requirements. If a player cannot be replaced, it means that the player does not have this rating.",
      ],
      ...
      "playerignore.popupm": [
        "配置点击调整后即保存，影响一键填充、阵容补全等处代码，切记谨慎选择。",
        "配置點選調整後即儲存，影響一鍵填充、陣容補全等處程式碼，切記謹慎選擇。",
        "The configuration is saved after clicking Adjust, which affects the code of one-click filling, lineup completion, etc. Remember to choose carefully.",
      ],
      ...
      "goldenplayer.popupm": [
        "默认黄金球员最高为83，如想设定请填入后点击确定，最小值为76。为空则恢复默认值。",
        "預設黃金球員最高為83，如想設定請填入後點選確定，最小值為76。為空則恢復預設值。",
        "The default gold player is up to 83. If you want to set it, please fill in and click OK. The minimum value is 76. If it is empty, restore the default value.",
      ],
      ...
      "headentrance.popupm": [
        "默认电脑端为5个、手机端为3个，请输入数字改变数量，最高不能超过8个。为空则恢复默认值。",
        "預設電腦端為5個、手機端為3個，請輸入數字改變數量，最高不能超過8個。為空則恢復預設值。",
        "The default is 5 on the computer and 3 on the mobile phone. Please enter the number to change the number, and the maximum cannot exceed 8. If it is empty, restore the default value.",
      ],
      ...
      "loas.input.tips": [
        "请按小时为基准填入，默认和1为1小时、3为3小时、6为6小时、12为12小时、24为1天、72为3天，不支持其他时间。",
        "請按小時為基準填入，預設和1為1小時、3為3小時、6為6小時、12為12小時、24為1天、72為3天，不支援其他時間。",
        "Please fill in the hours as the basis, the default and 1 is 1 hour, 3 is 3 hours, 6 is 6 hours, 12 is 12 hours, 24 is 1 day, 72 is 3 days, other times are not supported.",
      ],
      ...
      "notice.submitrepeat": [
        "阵容中有未分配不可交易版本，将自动替换并提交阵容。",
        "陣容中有未分配不可交易版本，將自動替換並提交陣容。",
        "If there is an unassigned non-tradable version in the lineup, it will be automatically replaced and submitted.",
      ],
      "fastsbc.popupt": ["快速任务提示", "快速任務提示", "Quick SBC Tip"],
      "fastsbc.popupm": [
        "此模式将快速执行指定SBC，优先未分配和进行排除选项，不会识别未分配可交易替换功能。此为实验功能谨慎使用，过量可能导致BAN提交等不知名惩罚，且可能提交掉你的有价值球员。确认后本次使用插件将不再提示。",
        "此模式將快速執行指定SBC，優先未分配和進行排除選項，不會識別未分配可交易替換功能。此為實驗功能謹慎使用，過量可能導致BAN提交等不知名懲罰，且可能提交掉你的有價值球員。確認後本次使用外掛將不再提示。",
      ...
      "plyers.bodytype.popupm": [
        "当前模型【%1】其表现为：%2，代表视觉感受的宽度和高度。可以理解为矮的腿短盘带好、高的腿长拦截好、瘦的窄灵活、壮的宽能撞。<br/><br/>定制体型不用高低区分都是扫描球员，静止时可能差距不大，但在运动中会更丝滑流畅或拥有专属击球、拦截、花式等动作。",
        "當前模型【%1】其表現為：%2，代表視覺感受的寬度和高度。可以理解為矮的腿短盤帶好、高的腿長攔截好、瘦的窄靈活、壯的寬能撞。<br/><br/>定製體型不用高低區分都是掃描球員，靜止時可能差距不大，但在運動中會更絲滑流暢或擁有專屬擊球、攔截、花式等動作。",
        "The body type [ %1 ] behaves as: %2 , which represents the width and height of visual perception. It can be understood as short legs with good dribbling, tall legs with good interception, lean narrow and flexible, and stocky wide can hit.<br/><br/>Unique body type does not need to be distinguished between tall and short. It may not look that different, but it will be silkier and smoother in motion or have exclusive hitting, intercepting, fancy and other actions.",
      ],
      ...
      "sbccount.popupm": [
        "此处仅计算插件运行状态下运行设备当日提交的SBC数量，请自行斟酌是否继续提交SBC。<br/>目前传闻是1小时内超过90个即有可能被禁止提交SBC，等待1到24小时解禁，具体规则EA未披露。",
        "此處僅計算外掛執行狀態下執行裝置當日提交的SBC數量，請自行斟酌是否繼續提交SBC。<br/>目前傳聞是1小時內超過90個即有可能被禁止提交SBC，等待1到24小時解禁，具體規則EA未披露。",
        "Here only the number of SBC submitted by the running device on the day when the plug-in is running is calculated. Please decide whether to continue submitting SBC. <br/> At present, it is rumored that more than 90 SBC submissions may be banned within 1 hours, and wait 1 to 24 hours for the ban to be lifted.",
      ],
      "meta.role.unknown": ["未知", "未知", "Unknown"],
      "plyers.relo.popupm": [
        "推荐职责【%1】，搭配默契风格【%2】，职责概述：<br/><br/>%3<br/><br/>%4<br/><br/>同职责满默契度级别：%5（%6），各级别含义：S（前1-10）、A（11-50）、B（51-100）、C（101-300）、D代表其他，门将因较少只到C级，？代表无数据。<br/><br/>仅评分前3000名的数据，+和++代表额外的熟悉度，可点击下方按钮前往查看。",
        "推薦職責【%1】，搭配默契風格【%2】，職責概述：<br/><br/>%3<br/><br/>%4<br/><br/>同職責滿默契度級別：%5（%6），各級別含義：S（前1-10）、A（11-50）、B（51-100）、C（101-300）、D代表其他，門將因較少只到C級，？代表無資料。<br/><br/>僅評分前3000名的資料，+和++代表額外的熟悉度，可點選下方按鈕前往檢視。",
      ...
      "plyers.relo.popupm.v2": [
        "职责可能在不同的位置存在，请根据位置、熟悉度等信息自行选择，并根据球员属性信息选择侧重点。",
        "職責可能在不同的位置存在，請根據位置、熟悉度等資訊自行選擇，並根據球員屬性資訊選擇側重點。",
        "Role may exist in different positions, please choose your own according to the positions, familiarity and other information, and choose the variations according to the player attribute information.",
      ],
      ...
      "trypack.foot.info3": [
        "此功能是通过EA公示概率模拟出的开启后获得的球员效果，此过程中不会与EA有数据交互，不会对你此后开包有所影响，仅供娱乐、切勿当真。",
        "此功能是通過EA公示概率模擬出的開啟後獲得的球員效果，此過程中不會與EA有資料互動，不會對你此後開包有所影響，僅供娛樂、切勿當真。",
        "This function is a player effect obtained after opening simulated by EA's publicity probability. It is for entertainment only and should not be taken seriously.",
      ],
      ...
      "shieldflag.popupm": [
        "此处开启将会使用此稀有度的球员（需开启可使用特殊球员按钮才可生效），点击右侧可切换状态，开关旁为此稀有度球员数。",
        "此處開啟將會使用此稀有度的球員（需開啟可使用特殊球員按鈕才可生效），點選右側可切換狀態，開關旁為此稀有度球員數。",
        "Open the player who will use this rarity here (you need to turn on the button to use rarity players to take effect), click on the right to switch the status, and the number of players with this rarity next to the switch.",
      ],
      ...
      "valuableplayer.popupm": [
        "发现提交阵容中拥有 %1 名珍贵球员（红色价格）显示，请决定是否继续提交。",
        "發現提交陣容中擁有 %1 名珍貴球員（紅色價格）顯示，請決定是否繼續提交。",
        "If the submitted lineup contains %1 valuable players (indicated in red), please decide whether to proceed with the submission.",
      ],
      ...
      "sbcneedslist.popupm": [
        "请注意此处计算不会计算周黑或特殊需求，仅计算每个需求阵容评分的SBC。<br>计算结果和价值均依托于评分FUTBIN最低价值，可能和实际使用略有偏差，仅供参考现有库存与实际完成的差异。",
        "請注意此處計算不會計算周黑或特殊需求，僅計算每個需求陣容評分的SBC。<br>計算結果和價值均依託於評分FUTBIN最低價值，可能和實際使用略有偏差，僅供參考現有庫存與實際完成的差異。",
      ...
      "realprob.popupm": [
        "此处为拉取FUTNEXT真实开包后的数据，可能与EA公布概率差距较大，数据仅供参考。<br>EA概率可能存在其未公布或无法匹配到，请见谅。",
        "此處為拉取FUTNEXT真實開包後的資料，可能與EA公佈概率差距較大，資料僅供參考。<br>EA概率可能存在其未公佈或無法匹配到，請見諒。",
        "This section pulls data from the real opening of FUTNEXT packs, which may significantly differ from the probabilities announced by EA. The data is provided for reference only. <br>EA may have unannounced or unmatched probabilities; please understand.",
      ],
      ...
      "openpack.result.popupm1": [
        "共开启 %1 个球员包（%2个未开启），分配俱乐部 %3 个、SBC仓库 %4 个，%5 个特别球员，最高评分 %6 。",
        "共開啟 %1 個球員包（尚有 %2 個未開啟），已分配至俱樂部 %3 個、SBC 倉庫 %4 個，%5 名特別球員，最高評分為 %6。",
        "Opened %1 player packs (%2 not opened), assigned %3 to Club, %4 to SBC storage, %5 special players, with a highest rating of %6.",
      ],
      "openpack.result.popupm2": [
        "上方将展示最多20位球员，优先展示特别品质和高评分的球员，其余球员球员将不会展示，请去俱乐部或SBC仓库自行查看。",
        "上方將展示最多 20 位球員，優先展示具備特殊品質與高評分的球員，其餘球員將不予顯示，請前往俱樂部或 SBC 倉庫自行查看。",
        "Up to 20 players will be displayed above, prioritizing special quality and high-rated players. Other players will not be shown — please check your Club or SBC storage for the rest.",
      ],
      ...
      "openpack.storebtn.popupm": [
        "批量开启将会自动开启指定球员包，非重复球员保存至俱乐部，重复且评分高于 %1(黄金范围) 的球员保存至SBC仓库，无法分配则弹出未分配列表并停止程序。<br><br>批量开启数量（默认为全部）：",
        "批量開啟將會自動開啟指定的球員包，非重複球員將保存至俱樂部，重複且評分高於 %1（黃金範圍） 的球員將保存至 SBC 倉庫，若無法分配，將彈出未分配列表並停止程序。<br><br>批量開啟數量（預設為全部）：",
      ...
      "accelerate.popupm": [
        "默契风格：%1  加速类型：%2<br><br>可改变加速类型默契：<br>",
        "默契風格：%1  加速類型：%2<br><br>可改變加速類型的默契風格：<br>",
        "Chemistry Style: %1  Acceleration Type: %2<br><br>Chemistry styles that can change acceleration type:<br>",
      ],
      "accelerate.popupm2": [
        "加速类型带有*标识未载入球员子属性计算存在偏差，可通过点击加速类型标识或球员简历处载入属性后矫正数值。",
        "加速類型帶有 * 標誌表示尚未載入球員子屬性，計算結果可能存在偏差。可點擊加速類型標誌或球員履歷以載入屬性並校正數值。",
        "Acceleration types marked with * indicate that player sub-attributes have not been loaded. This may result in inaccurate calculations. Click the acceleration type label or the player profile to load attributes and correct the values.",
      ],
      ...
      "pickpreview.popupm": [
        "请注意此处仅为展示挑选球员的预览，并非全部获得。且可能存在数据差异，请以游戏本身效果为准。",
        "請注意，此處僅為展示可挑選球員的預覽，並非實際全部獲得的內容。且可能存在資料差異，請以遊戲內的實際效果為準。",
        "Please note that this is only a preview of the selectable players and does not represent all obtained items. Some data discrepancies may occur; please refer to the in-game results as the final standard.",
      ],
    };
    //固话的HTML内容
    html = {
    ...
    events.getPriceForUrl = async (data) => {
      ...
      try {
        ...
        if (info.base.platform !== "pc") {
          ...
          const response = await events.externalRequest("GET", "https://www.fut.gg/api/fut/player-prices/26/?ids=" + params);
          ...
          _.map(originalJson.data, (i) => {
            ...
            if (p) {
              priceJson[i.eaId] = {
                ...
                y: i.isSbc || i.isObjective || i.isUntradeable ? 2 : 1,
              };
            } else {
              priceJson[i.eaId] = {
                n: 0,
          ...
          //https://enhancer-api.futnext.com/players/prices?ids=50541905_226853_203376_247103_268889_246104_50564135_238616_242964_256675_231677_1271639_234396_241084_50598566_239580_262457_259372_243014_241852_50575228_76687_227190_261773&platform=pc
    ...
    UTUnassignedItemsViewController.prototype.updateUntradeableDuplicateSectionOptions = function (...args) {
      ...
      //25.24 卡重复自动刷新
      const duplicateIds = _.filter(_.map(this.viewmodel.values(), "duplicateId"), (id) => id !== 0);
      if (
        duplicateIds.length &&
        duplicateIds.length !== events.getItemBy(2, { id: duplicateIds }).length &&
        section &&
        !this.getView().getSection(UTUnassignedItemsViewModel.SECTION.ITEMS) &&
        !this.getView().getSection(UTUnassignedItemsViewModel.SECTION.DUPLICATES) &&
        _.has(section, "_fsuGoToStorage")
      ) {
    ...
    UTUnassignedItemsView.prototype.renderSection = function (e, t, i) {
      ...
      if (t == UTUnassignedItemsViewModel.SECTION.UNTRADABLEDUPLICATES) {
        ...
        if (players.length) {
          ...
          //25.21 高分球员存入仓库按钮
          const hPlayers = _.orderBy(
            _.filter(players, (i) => i.rating > info.set.goldenrange),
            ["rating"],
            ["desc"]
          );
          ...
          if (_.size(services.SBC.repository.getSets())) {
            ...
            _.forOwn(fastList, (i) => {
              ...
              if (set && !set.isComplete() && (challenge == null || !challenge.isCompleted())) {
                ...
                btnTitle = _.has(info.base.fastsbc[i.n], "n") ? `${info.base.fastsbc[i.n].n}` : `${set.name}-${i.cId}`;
    ...
    UTSquadBuilderViewController.prototype.viewDidAppear = function () {
      ...
      if (this.squad && this.squad.isSBC()) {
        ...
        this.getView()._fsuleague = events.createToggle(`${fy(`builder.league`)}(${info.set.shield_league.length})`, async (e) => {
    ...
    UTPlayerItemView.prototype.renderItem = function (p, t) {
      ...
      if (p.isValid()) {
        setTimeout(() => {
          ...
          if (pid === p.id) {
            if (!_.has(this, "_fsuSquadIndex") || this._fsuSquadIndex === idx) return;
            this._fsuSquadIndex = idx;
          }
          ...
          let otherPos = p.possiblePositions
            .filter((z) => {
              return z !== p.preferredPosition;
            })
            .map((z) => {
              return UTLocalizationUtil.positionIdToName(z, services.Localization);
            });
          ...
          posElement.innerHTML = otherPos
            .map((z) => {
              return `<div>${z}</div>`;
            })
            .join(``);
          this._fsuCardPos = posElement;
          ...
          if (!p.isGK() && !isSmall) {
            ...
            this._fsuaccele = events.createButton(
              new UTButtonControl(),
            ...
            this._fsuaccele.getRootElement().setAttribute("data-defid", p.definitionId);
            this._fsuaccele.getRootElement().style.cursor = `pointer`;
            extraElement.appendChild(this._fsuaccele.getRootElement());
          }
          ...
          if (bodyTypeId) {
            this._fsubodytype = events.createButton(
              ...
              "",
              async (e) => {
                events.popup(fy("plyers.bodytype.popupt"), fy(["plyers.bodytype.popupm", info.bodytypetext[bodyTypeId], fy(`players.bodytype_${bodyTypeId}`)]), (t) => {});
              },
              ""
            );
            this._fsubodytype.getRootElement().style.cursor = `pointer`;
            this._fsubodytype.getRootElement().innerHTML = _.replace(info.bodytypetext[bodyTypeId], "&", `<span style='font-size:80%'>&</span>`);
            extraElement.appendChild(this._fsubodytype.getRootElement());
          }
          this._fsuCardAttr = extraElement;

          if (!isSmall) {
            let realFace = _.includes(info.meta.realFace, p.definitionId) ? 0 : 1;
            this._fsuRealFace = events.createButton(
              new UTButtonControl(),
              realFace == 0 ? "YES" : "NO",
              async (e) => {
            ...
            extraElement.appendChild(this._fsuRealFace.getRootElement());
          }

          //24.18 可进化标识：计算展现标识数据
          if (p.loans === -1 && !isSmall) {
            let academyNumber = 0;
          ...
          const isSold = p.getAuctionData().isSold();

          this._fsuCardPrice = events.createElementWithConfig("div", {
            classList: ["fsu-cards-price", "fsu-price-box", "fsu-price-val", "fsu-cards", stc, ppInCache && ppValue.type == 2 ? "fsu-price-reward" : "currency-coins"],
            style: {
              borderColor: p.untradeableCount && !isSold ? "rgba(247, 183, 2, 0.6)" : "rgba(255, 255, 255, 0.6)",
              color: p.untradeableCount && !isSold ? "#f7b702" : "",
            },
            textContent: ppValue.text,
            attributes: {
              "data-id": p.definitionId,
              "data-value": ppValue.num,
              "data-type": ppValue.type,
            },
          });

          /** 25.18 珍贵球员提示 **/
          let preciousPlayerSquadController = isPhone() ? cntlr.current() : cntlr.left();
          if (preciousPlayerSquadController?.className == "UTSBCSquadOverviewViewController" && events.getCachePrice(p.definitionId, 1).num) {
            if (!p.isSpecial() & _.gte(events.getCachePrice(p.definitionId, 1).num, 2 * info.base.price[p.rating])) {
          ...
          if (_.has(p, "_fsuPrice") && _.has(p, "_fsuClosing")) {
            ...
            const hasLastPrice = p.lastSalePrice !== 0;
            this._fsuCardPriceBox = events.createElementWithConfig("div", {
              classList: ["fsu-price-box"],
              attributes: {
                "data-id": p.definitionId,
              },
            });
            // 若非手机并且有上次价格，加入 lastSale 区块
            if (!isPhone() && hasLastPrice) {
              const last = events.createElementWithConfig("div", {
                classList: ["fsu-price-last"],
                innerHTML: `
                                    <div class="title">${fy("price.last")}</div>
                                    <div class="value currency-coins">${p.lastSalePrice.toLocaleString()}</div>
                                `,
              });
              this._fsuCardPriceBox.appendChild(last);
            }

            // 创建当前价格区块
            const priceYield = hasLastPrice ? events.priceLastDiff(ppValue.num, p.lastSalePrice) : "";
            const priceNowBox = events.createElementWithConfig("div", {
              classList: ["fsu-price-val"],
              attributes: {
                "data-value": ppValue.num,
                "data-type": ppValue.type,
                ...(isPhone() && hasLastPrice ? { "data-last": p.lastSalePrice } : {}),
              },
              style: {
                color: p.untradeableCount && !isSold ? "#f7b702" : "",
              },
              innerHTML: `
                                <div class="title">${fy("price.now")}${priceYield}</div>
                                <div class="value ${ppInCache && ppValue.type == 2 ? "fsu-price-reward" : "currency-coins"}">${ppValue.text}</div>
                            `,
            });
            this._fsuCardPriceBox.appendChild(priceNowBox);
          }

          let plow =
            info.base.price.hasOwnProperty(p.rating) && p.rating > info.base.price.low && p.rating < info.base.price.high
              ? `<div class="fsu-other-low currency-coins">${p.rating} Min : ${Number(info.base.price[p.rating]).toLocaleString()}</div>`
              : `<span class="fsu-other-low"></span>`;

          let pOtherPos = otherPos.length ? `<div class="fsu-other-pos">${otherPos.join(" / ")}</div>` : `<span class="fsu-other-pos"></span>`;

          let pe = -1,
            sp = events.getItemBy(2, { definitionId: p.definitionId });
          if (sp.length == 1) {
          ...
          if (info.set.card_meta) {
            ...
            if (isSmall) {
              this._fsuCardAttr.prepend(this._fsuCardMetaRating);
            }
          ...
          if (!this.__root.querySelector("span[p-id]")) {
            ...
            if (parentNode?.classList.contains("ut-squad-slot-view")) {
              this._fsuSquadIndex = Number(parentNode?.getAttribute("index")) || 0;
            }
          ...
          if (isSmall) {
            ...
            if (cs !== 0) {
              ...
              if ([5, 21, 22, 23, 24, 25, 31].includes(cs)) {
                if (cs == 21) {
                  ...
                  if (posElement) {
                    // 25.22 添加阵容直接移除球员按钮
                    let removeBtn = events.createButton(
                      new UTImageButtonControl(),
                      "",
                      (e) => {
                        events.showLoader();
                        let newSquad = _.cloneDeep(_.last(cntlr.current()._squad._fsuOldSquad));
                    ...
                    Object.assign(removeBtn.getRootElement().style, {
                      margin: "-6px",
                      fontSize: "12px",
                    });
                    posElement.appendChild(removeBtn.getRootElement());
                  }
                  if (!p.untradeableCount) {
                ...
                if (cs == 12 || cs == 11) {
                  ...
                  parentNode.append(this._fsuCardPriceBox);
                ...
                if (![7, 4].includes(cs)) {
                  parentNode.append(this._fsuCardOther);
                }
              ...
              if (cs == 25 && _.has(this, "_fsuCardMeta")) {
                this._fsuCardMeta.getRootElement().style.height = ".8rem";
                this._fsuCardMeta.getRootElement().style.fontSize = ".6rem";
                this.__root.after(this._fsuCardMeta.getRootElement());
                this._fsuCardPrice.style.fontSize = ".8rem";
                this._fsuCardPrice.style.marginTop = ".2rem";
              }
            }
          } else {
            ...
            if (cardParen.classList.contains("reward")) {
              this._fsuCardMeta.getRootElement().style.bottom = "2.2rem";
            }
    ...
    UTSquadOverviewViewController.prototype.viewDidAppear = function () {
      ...
      if (this._squad.isSBC()) {
        ...
        if (_.size(info.base.fastsbc) > 0) {
          ...
          if (q) {
            console.log(q);
            let qs = events.fastSBCQuantity(
              true,
              _.filter(repositories.Item.getUnassignedItems(), (item) => item.isPlayer() && item.duplicateId !== 0),
              q
            );
            if (qs) {
        ...
        //开始判断是否需要屏蔽搜索按钮
        let transferIds = _.uniq(
          _.map(repositories.Item.getTransferItems(), (i) => {
            if (i.getAuctionData().isInactive()) {
              return i.definitionId;
            }
          }).filter(Boolean)
        );
        ...
        if (info.set.sbc_sback) {
          let rb = events.createButton(
            ...
            (e) => {
              ...
              if (c) {
                events.popup(fy("squadback.popupt"), fy(["squadback.popupm", c]), (t) => {
                  if (t === 2) {
                    events.showLoader();
    ...
    UTSectionedItemListView.prototype.addItems = function (t, e, i, r) {
      ...
      if (
        info.set.player_loas &&
        services.User.getUser().tradeAccess == TradeAccessLevel.ALLOWED &&
        cntlr.current().getNavigationTitle() !== services.Localization.localize("navbar.label.watchlist") &&
        (cntlr.current().getNavigationTitle() !== services.Localization.localize("navbar.label.assigncards") || repositories.Item.getPileSize(ItemPile.TRANSFER) - repositories.Item.numItemsInCache(ItemPile.TRANSFER) > 0)
      ) {
        let pn = 0,
          pr = {},
          ln = 0;
        ...
        if (pn) {
          ...
          const playerIds = _.chain(this.listRows)
            .filter((row) => row.data.type === "player" && !events.getCachePrice(row.data.definitionId, 3))
            .map((row) => row.data.definitionId)
            .value();
    ...
    events.ignorePlayerTypePopup = (type) => {
      ...
      const optionData = _.orderBy(
        config.factories.map((f) => ({
          name: f.label,
          id: f.id,
      ...
      _.forEach(optionData, (o) => {
        let oToggle = events.createToggle(o.name, async (e) => {
          if (e.getToggleState()) {
    ...
    events.ignorePlayerPopup = () => {
      ...
      mp.onExit.observe(mp, (e, z) => {
        ...
        if (z == 44404) {
          ...
          events.popup(
            ...
            [{ labelEnum: enums.UIDialogOptions.OK }, { labelEnum: 44403 }],
      ...
      _.forEach(buildArray, (b) => {
        let bText = getText(b);
        let bToggle = events.createToggle(bText, async (e) => {
    ...
    events.popup = (t, m, c, o, i, n, s) => {
      if (!o) {
        o = [{ labelEnum: enums.UIDialogOptions.OK }, { labelEnum: enums.UIDialogOptions.CANCEL }];
      }
      let message = m;
      if (info.isEnhancer) {
    ...
    events.getCachePrice = (i, t) => {
      ...
      if (t) {
        ...
        return _.has(info.roster[priceDataKey], i) && item?.n !== 0;
      }
    };
    ...
    UTSquadEntity.prototype.getRating = function () {
      ...
      events.squadTotal(_.compact(_.map(this.getFieldPlayers(), (i) => (events.getCachePrice(i.item.definitionId) ? i.item.definitionId : null))));
      return r;
    };
    ...
    events.squadTotal = (p) => {
      ...
      if (e) {
        let n = 0;
        for (let i of p) {
          n += events.getCachePrice(i, 1).num;
        }
        e.querySelector("#squadTotalValue").innerText = n.toLocaleString();
        if (n !== 0 && document.getElementById("sbcPrice")) {
          let p = e.querySelector(".fsu-squad-pTitle");
    ...
    events.getPriceForFubin = async (pid) => {
      try {
        ...
        const priceJson = {
          ...
          y: originalJson.MinPrice || originalJson.MaxPrice ? 1 : 2,
        };
    ...
    events.loadPlayerInfo = async (items, el, type) => {
      ...
      if (list.length > 0) {
        ...
        for (let k in pu) {
          ...
          try {
            if (type) {
              playerPrice = Object.fromEntries(await Promise.all(pu[k].map(async (i) => [i, await events.getPriceForFubin(i)])));
          ...
          _.map(playerPrice, (v, k) => {
            ...
            if (e.length > 0) {
              for (let i of e) {
                const ppValue = events.getCachePrice(k, 1);
                const isDirect = i.classList.contains("fsu-price-val");
                const priceEl = isDirect ? i : i.querySelector(".fsu-price-val");
                const valueEl = isDirect ? i : i.querySelector(".fsu-price-val .value");

                if (priceEl && valueEl) {
                  priceEl.setAttribute("data-value", ppValue.num);
                  priceEl.setAttribute("data-type", ppValue.type);
                  valueEl.innerText = ppValue.text;

                  if (ppValue.type == 2) {
                    valueEl.classList.remove("currency-coins");
                    valueEl.classList.add("fsu-price-reward");
                  }
                }
                if (isDirect) {
                  // 珍贵球员逻辑
                  const preciousPlayerSquadController = isPhone() ? cntlr.current() : cntlr.left();

                  if (preciousPlayerSquadController?.className === "UTSBCSquadOverviewViewController") {
                    const player = events.getItemBy(2, { definitionId: Number(k) });
                    const playerObj = player[0];
                    const basePrice = info.base.price?.[playerObj?.rating];
                    if (player.length && !playerObj.isSpecial() && _.gte(ppValue.num, 2 * basePrice)) {
                      i.style.backgroundColor = "#9f1d00";
                      const currentList = _.isArray(preciousPlayerSquadController._fsuValuable) ? preciousPlayerSquadController._fsuValuable : [];
                      preciousPlayerSquadController._fsuValuable = _.union(currentList, [Number(k)]);
                    }
                  }
                }
                ...
                if (i.querySelectorAll(lastPriceName).length > 0) {
                  i.querySelector(".fsu-price-val .title span").outerHTML = events.priceLastDiff(
                    v.n,
                    isPhone() ? i.querySelector(lastPriceName).getAttribute("data-last").replace(/,/g, "") : i.querySelector(lastPriceName).innerText.replace(/,/g, "")
                  );
                }
              }
            }
        ...
        if (document.getElementById("squadTotal")) {
          events.squadTotal(
            cntlr
              .current()
              ._squad.getFieldPlayers()
              .map(function (i) {
                return i._item.definitionId;
              })
              .filter((i) => i > 0)
          );
        }
    ...
    UTBaseSquadSplitViewController.prototype.viewDidAppear = function () {
      ...
      if (info.set.info_squad) {
        let p = this._squad
          .getPlayers()
          .map(function (i) {
            if (i._item.type == "player" && !events.getCachePrice(i._item.definitionId).num) {
              return i._item.definitionId;
            }
          })
          .filter((i) => i > 0);

        let squadElement = events.createElementWithConfig("div", {
          classList: ["ut-squad-summary-info"],
          style: {
            flex: "0",
            paddingLeft: "0",
          },
        ...
        squadElement.appendChild(
          events.createElementWithConfig("h1", {
            textContent: fy("sbc.topsquad"),
            classList: ["ut-squad-summary-label", "fsu-squad-pTitle"],
          })
        );
        squadElement.appendChild(events.createElementWithConfig("span", {}));
        let squadValueElement = events.createElementWithConfig("div", {
          classList: ["rating-inline"],
        });
        squadValueElement.appendChild(
          events.createElementWithConfig("span", {
            textContent: "0",
            classList: ["fsu-squad-pValue", "currency-coins"],
            attributes: {
              id: "squadTotalValue",
            },
          })
        );
        squadElement.appendChild(squadValueElement);
        this.leftController
          .getView()
          .getSummaryPanel()
          .getRootElement()
          .querySelector(`.ut-squad-summary-info${this.className == "UTSBCSquadSplitViewController" ? "--right" : ""}`)
          .after(squadElement);
        ...
        if (!p.length) {
          events.squadTotal(
            this._squad
              .getFieldPlayers()
              .map(function (i) {
                if (events.getCachePrice(i._item.definitionId).num) {
                  return i._item.definitionId;
                }
              })
              .filter((i) => i > 0)
        ...
        if (this._squad.isSBC()) {
          ...
          if (si !== -1 && !document.getElementById("sbcPrice")) {
            let sbcSquadElement = squadElement.cloneNode(true);
            sbcSquadElement.querySelector(".fsu-squad-pValue").id = "sbcPrice";
            sbcSquadElement.querySelector(".fsu-squad-pValue").textContent = Number(info.base.platform == "pc" ? info.task.sbc.stat[this._set.id].pc : info.task.sbc.stat[this._set.id].tv).toLocaleString();
            squadElement.style.paddingRight = "0";
            squadElement.after(sbcSquadElement);
            Object.assign(this.leftController.getView().getSummaryPanel().getRootElement().querySelector(".chemistry").style, {
              display: "flex",
              flexWrap: "nowrap",
              flexDirection: "row",
              alignItems: "center",
            ...
            this.leftController.getView().getSummaryPanel()._squadStats.getShowChemistryButton().getRootElement().style.height = "auto";
            this.leftController.getView().getSummaryPanel()._squadStats.getRootElement().style.flex = "1";
          }
        }
      }
    };
    ...
    events.SBCSetRatingPlayers = async (e) => {
      ...
      switch (queryType) {
        case "d":
          pendingPlayers = repositories.Item.getUnassignedItems().map((i) => {
        ...
        if (info.set.info_sbc) {
          _.map(e, (i, k) => {
            ...
            if (catNewIds.length || catExpiryIds.length) {
              const realNewCount = _.filter(catNewIds, (id) => !services.SBC.repository.getSetById(id).isComplete()).length;
              const realExpiryCount = _.filter(catExpiryIds, (id) => !services.SBC.repository.getSetById(id).isComplete()).length;
    ...
    UTAcademyHubViewController.prototype.onRequestHubDataComplete = function (e, t) {
      t.data.slots = _.orderBy(
        t.data.slots,
        [(item) => (info.evolutions.new.includes(item.id) ? 0 : 1), (item) => (item.endTimePurchaseVisibility === 0 ? Infinity : item.endTimePurchaseVisibility), (item) => (item.endTime === 0 ? Infinity : item.endTime)],
        ["asc", "asc", "asc"]
      );
        ...
        if (item.id !== -1) {
          ...
          if (academyCategory) {
            ...
            const expiryAcademy = _.filter(academyCategory.slotIds, (id) => {
              ...
              let endTime = academySlot?.status == AcademySlotState.NOT_STARTED ? _.min(_.filter([academySlot.endTime, academySlot.endTimePurchaseVisibility], (v) => v && v !== 0)) : academySlot?.endTime;
    ...
    UTAcademyHubView.prototype.generateSlot = function (e) {
      ...
      if (info.evolutions.new.includes(e.id)) {
        isChange = true;
        tileViewRoot.appendChild(events.createDF(`<div class='fsu-newtips'>${fy("task.new")}</div>`));
      }
      const endTime = e.status == AcademySlotState.NOT_STARTED ? _.min(_.filter([e.endTime, e.endTimePurchaseVisibility], (v) => v && v !== 0)) : e.endTime;
      if (endTime && endTime - Math.round(new Date() / 1000) < 172800) {
        isChange = true;
        tileViewRoot.prepend(events.createDF(`<div class='task-expire'>${fy("task.expire")}</div>`));
        ...
        tileViewRoot.querySelector(".ut-academy-slot-tile-view--title").style.marginTop = "16px";
      }
      if (isChange) {
    ...
    UTAcademyPlayerFromClubViewController.prototype.requestItems = function (...args) {
      ...
      if (_.size(localSearch)) {
        ...
        console.log(result);
        this.handleItemRetrieval(
          _.filter(result, (player) => {
            return this.academySlot.meetsRequirements(player);
          }),
          true
        );
      } else {
    ...
    UTSBCHubView.prototype.populateTiles = function (e, t) {
      ...
      const newList = _.orderBy(
        e,
        [(item) => item.isComplete(), (item) => _.includes(info.task.sbc.stat.new, item.id) || (!_.has(info.task.sbc.stat, item.id) && item.id !== 1), (item) => info?.task?.sbc?.stat?.[item.id]?.u ?? -Infinity],
        ["asc", "desc", "desc"]
      );
    ...
    events.sbcFilter = (e) => {
      let t = cntlr.current().getView().sbcSetTiles,
        g = e.getIndex();
      for (let i of t) {
        let y = true,
          d = i.data.id;
      ...
      if ("fsuAcademy" in this && this.fsuAcademy) {
        ...
        if (!("fsuAcademyChange" in this)) {
          ...
          if (_.isObject(this.fsuAcademy)) {
            let coins = this.fsuAcademy.getPrice(GameCurrency.COINS),
              points = this.fsuAcademy.getPrice(GameCurrency.POINTS);
          ...
          if (_.isObject(this.fsuAcademy)) {
            this.fsuAcademy.status === AcademySlotState.NOT_STARTED &&
              ((this.fsuAcademy.player = new UTNullItemEntity()),
    ...
    UTRewardSelectionChoiceView.prototype.expandRewardSet = function (e, t) {
      ...
      let reward = t.rewards.find((i) => i.count),
        tn = this._rewardsCarousel._tnsCarousel.__root;
    ...
    events.detailsButtonSet = (e) => {
      ...
      if (e.item.isPlayer()) {
        ...
        if (pid && e.item.concept && "_fsuConceptBuy" in a && events.getCachePrice(pid) && info.set.sbc_conceptbuy) {
          ...
          a._fsuConceptBuy.setSubtext(events.getCachePrice(pid, 1).num);
        ...
        //24.16 修复返回问题
        if (
          a.hasOwnProperty("_sendClubButton") &&
          w.getParentViewController().className == `UTItemDetailsNavigationController` &&
          w.getParentViewController()._squadContext &&
          a._sendClubButton.isInteractionEnabled() &&
          e.item.definitionId == w.getParentViewController()._squadContext.squad.getPlayer(w.getParentViewController()._squadContext.slotIndex).item.definitionId &&
          w.getParentViewController()._squadContext.squad.getPlayer(w.getParentViewController()._squadContext.slotIndex).item.concept &&
          info.set.sbc_cback
        ) {
          events.conceptBuyBack(w);
          return;
        ...
        if (events.getCachePrice(pid)) {
          let lp = events.getCachePrice(pid, 1).text;
          if (lp && lp !== "0" && lp !== "Reward") {
        ...
        //24.18 可进化标识：查看按钮添加
        if (e.item.loans === -1 && e.item.id && e.item.isPlayer() && !a.hasOwnProperty("_fsuAcademyHtml")) {
          ...
          if (academy.length) {
            ...
            _.forEach(academy, (i) => {
                ...
                (e) => {
                  ...
                  academyViewmodel.getSlotPreview(e.academy.id, e.player).observe(targetView, function (e, t) {
                    if ((e.unobserve(targetView), t.success && JSUtils.isObject(t.data))) {
                      ...
                      selectedAcademy.nowPlayer = nowPlayer;
                      ...
                      (r = NetworkErrorManager.checkCriticalStatus(t.status)
                        ? (NetworkErrorManager.handleStatus(t.status), s.localize("academy.preview.loaderror"))
                        : t.status === HttpStatusCode.SERVICE_UNAVAILABLE_ERROR || (null === (i = t.error) || void 0 === i ? void 0 : i.code) === UtasErrorCode.SERVICE_IS_DISABLED
                        ? s.localize("academy.feature.disabled")
                        : s.localize("academy.preview.unexpectederror")) && a.queue([r, UINotificationType.NEGATIVE]);
                    }
                  });
                },
                "accordian"
              ...
              academyBut.parent = w;
              academyBut.item = e.item;
              academyBut.setDisplay(0);
              if (!_.size(events.getItemBy(2, { id: i.player }))) {
      ...
      if (a.hasOwnProperty("_fsuSwap") && a.hasOwnProperty("_fsuUn")) {
        ...
        let ul = cntlr
          .current()
          ._squad._players.map(function (i) {
            if (i.item.definitionId > 0) {
              return i.item.definitionId;
            }
          })
          .filter(Boolean);
        let rul = repositories.Item.getUnassignedItems()
          .map((i) => {
            if (i.isDuplicate() && !i.isLimitedUse() && i.isPlayer()) {
              if (ul.length) {
      ...
      let pa = events.createButton(
        ...
        (e) => {
          ...
          let p = Number(e.__subtext.innerText.replace(/,/g, "")),
            i = Number(e.__subtext.getAttribute("data-i"));
          ...
          async () => {
            ...
            let p = events.getDedupPlayers(
              events.getItemBy(
                ...
                {
                  definitionId: _.map(
                    _.filter(repositories.Item.getUnassignedItems(), (i) => i.isDuplicate() && !i.isLimitedUse() && i.isPlayer()),
                    "definitionId"
                  ),
                  upgrades: null,
                },
    ...
    events.SBCSetMeetsPlayers = async (e) => {
      ...
      let defList = e._parent.challenge.squad
        .getPlayers()
        .map((i) => {
          return i.getItem().definitionId;
        })
        .filter(Boolean);
      ...
      let currentList = newChallenge.squad.getPlayers().map((i) => {
        return i.getItem();
      });
      let resultList = [];
    ...
    events.SBCSetChemPlayers = async (e) => {
      ...
      let currentList = e._parent.squad.getFieldPlayers().map((i) => {
        return i.item;
      });
      let defList = e._parent.viewmodel.values().map((i) => {
        return i.item.definitionId;
      });
    ...
    UTHomeHubView.prototype._generate = function (...args) {
      if (!this._generated) {
        ...
        this._fsuDodo = events.createTile(fy("tile.dodotitle"), fy("tile.dodotext"), (e) => {
          GM_openInTab(`https://fut.to`, { active: true, insert: true, setParent: true });
        });
        this._sbcTile.__root.after(this._fsuDodo.__root);
        this._fsuSet = events.createTile(fy("tile.settitle"), fy("tile.settext"), (e) => {
        ...
        this._fsuGP = events.createTile(fy("tile.gptitle"), fy("tile.gptext"), (e) => {
    ...
    events.reloadPlayers = async (e) => {
      ...
      await services.Club.getStats().observe(current, async function _onGetStats(e, t) {
        e.unobserve(current);
        t.success
          ? t.response.stats.forEach(async function (e) {
              if (e.type == "players") {
                if (e.count !== services.Club.clubDao.clubRepo.items.length) {
                  ...
                  if (cntlr.current().className == "UTHomeHubViewController" && info.task.obj.html && cntlr.current().getView()._objectivesTile.__tileContent.querySelector(".ut-tile-view--subtitle")) {
                    if (!cntlr.current().getView()._objectivesTile.__root.querySelector(".fsu-task")) {
                      cntlr
                        .current()
                        .getView()
                        ._objectivesTile.__tileContent.before(events.createDF(`<div class="fsu-task">${info.task.obj.html}</div>`));
                    }
                  ...
                  if (
                    ...
                    cntlr.current().getView()._sbcTile.__tileContent.querySelector(".ut-tile-content-graphic-info")
                  ) {
                    cntlr
                      .current()
                      .getView()
    ...
    UTHomeHubView.prototype.getObjectivesTile = function () {
      if (info.task.obj.html && !this._objectivesTile.__root.querySelector(".fsu-task") && info.set.info_obj) {
        this._objectivesTile.__tileContent.before(events.createDF(`<div class="fsu-task">${info.task.obj.html}</div>`));
      }
    ...
    UTHomeHubView.prototype.getSBCTile = function () {
      if (info.set.info_sbc && info.task.sbc.html && !this._sbcTile.__root.querySelector(".fsu-task")) {
        this._sbcTile.__tileContent.before(events.createDF(`<div class="fsu-task">${info.task.sbc.html}</div>`));
      }
      ...
      if (_.has(e, "_fsuFiltersCount")) {
        ...
        if (basePlayers.length) {
          ...
          for (let [index, element] of list.entries()) {
            let id = e.options[index].id,
              count = resultMap.get(`${id}`);
            if (count) {
              if (isPhone()) {
    ...
    events.searchFill = async (e) => {
      let c = e.viewmodel.searchCriteria,
        t = e.parentViewController._fsuFillType,
          ...
          if (info.base.input) {
            ...
            e.target._oData.forEach(function (el, i) {
              let a = document.querySelectorAll(p)[i],
                c = info.set.sbc_icount && "_oCount" in e.target ? (e.target._oCount[i] >= Number(v) ? true : false) : false;
              if (el.includes(v) || c) {
          ...
          if (solePlayers.length && info.set.player_transfertoclub) {
            ...
            controller._fsuSendClub = events.createButton(
              ...
              async (e) => {
                ...
                events.popup(fy("transfertoclub.popupt"), fy(["transfertoclub.popupm", e.list.length]), (t) => {
                  if (t === 2) {
      ...
      services.Item.move(list, ItemPile.CLUB).observe(controller, (e, t) => {
        if ((e.unobserve(controller), t.success)) {
          ...
          t.data.untradeableSwap
            ? services.Notification.queue([services.Localization.localize("notification.item.moveFailed"), UINotificationType.NEGATIVE])
            : (services.Notification.queue([services.Localization.localize("notification.item.moveFailed"), UINotificationType.NEGATIVE]), NetworkErrorManager.handleStatus(t.status));
        }
      });
    };
    //快速SBC数量检测
    events.fastSBCQuantity = (y, p, o) => {
      ...
      if (!info.build.strictlypcik && events.isEligibleForOneFill(o)) {
        ...
        if (!y && _.size(tempCriterias) === 2 && _.get(tempCriterias[0], "t.rs") === _.get(tempCriterias[1], "t.rs")) {
    ...
    UTSquadBuilderViewModel.prototype.generatePlayerCollection = function (e, o, n, r) {
      ...
      let pa = e.map(function (_, t) {
        ...
        return i && (i.isValid() || i.isBrick()) ? i.getItem() : info.build.ignorepos ? p[c++] : s.getBestPlayerForPos(_, p);
      });
    ...
    fsuSV.prototype._generate = function _generate() {
      if (!this._generated) {
        ...
        this._cStyle = {};
        this._cStyle.new = events.createToggle(fy(`set.style.new`), async (e) => {
        ...
        this._cStyle.old = events.createToggle(fy(`set.style.old`), async (e) => {
    ...
    set.addToggle = function (na, nb) {
      let e = events.createToggle(fy(`set.${na}.${nb}`), async (e) => {
    ...
    set.init = function () {
      let a = JSON.parse(GM_getValue("set", "{}")),
        b = {};
    ...
    events.getAuction = async function (e) {
      ...
      let id = e.__root.getAttribute("data-id"),
        p = e.__root.getAttribute("data-p").replace(/,/g, ""),
        p_result,
        p_arr = [],
        p_cm = {},
        p_cmarr = [],
      ...
      if (Object.keys(p_cm).length) {
        ...
        if (p_e) {
          ...
          for (let i in p_cm) {
            n++;
            let l_e = events.createButton(new UTGroupButtonControl(), `${fy("quicklist.getpricelt")} ${n}`, () => {}, "accordian fsuGPL");
    ...
    events.getCardTipsHtml = (type) => {
      ...
      const { tipsClass, tipsIcon } = configMap[type] || configMap[1]; // 默认使用 type=1 配置
    ...
    events.isEligibleForOneFill = (obj) => {
      ...
      if (_.size(obj) !== 2 || !_.every(obj, (o) => _.isEqual(_.sortBy(_.keys(o.t)), _.intersection(_.keys(o.t), allowedKeys).sort()))) {
        return false;
      }
    ...
    UTSBCSquadDetailPanelView.prototype.render = function (e, t, i, o) {
      ...
      const rewardElement = this._challengeDetails._groupRewardList.__root;
      const exchangeElement = this._btnExchange.__root;
      ...
      if (!this._fsuBIB) {
        this._fsuBIB = events.createButton(
          ...
          (e) => {
            let conceptPlayers = _.map(
              _.filter(e.challenge.squad.getPlayers(), (slot) => {
      ...
      if (!this._fsuIgnore) {
        ...
        Object.assign(this._fsuIgnore.__root.style, {
          width: "calc(100% - 1rem)",
          marginTop: ".675rem",
          margin: ".5rem auto",
        });
        rewardElement.appendChild(this._fsuIgnore.__root);
      ...
      if (Object.keys(gf).length) {
        ...
        if (fastSbcNeedInfo.length) {
          ...
          Object.assign(fastSbcEditBtn.__root.style, {
            width: "calc(100% - 1rem)",
            marginTop: ".675rem",
            margin: ".5rem auto",
          });
          this._fsuSbcEdit = fastSbcEditBtn;
          rewardElement.appendChild(this._fsuSbcEdit.__root);
        }
        ...
        if (!this._fsuAutoFill && info.set.sbc_autofill) {
            ...
            (e) => {
              ...
              if (!info.build.strictlypcik && events.isEligibleForOneFill(e._gf)) {
                ...
                for (let i of e._gf) {
                  ...
                  gf["lock"] = false;
                  // 25.23 移除优先仓库强制移除的判定。
                  // if(_.has(gf,"GTrating")){
                  //     gf["firststorage"] = false;
                  // }
                  let result = events.getItemBy(2, gf, repositories.Item.getUnassignedItems());
                  //console.log(result,_.map(result,"rating"))
                  let z = result.slice(0, i.c);
                  console.log(z, _.map(z, "rating"));
                  gtb = gtb.concat(
                    z.map((i) => {
                      return i.databaseId;
                    })
          ...
          exchangeElement.before(this._fsuAutoFill.__root);
          this._fsuIgnore.show();
          events.sbcFilterTipsGenerate("_fsuIgnore", this, 2);
          events.sbcFilterTipsGenerate("_fsuIgnore", this, 3);
        }
        ...
        let b = events.createButton(
          ...
          (e) => {
            const dupIds = _.map(
              _.filter(repositories.Item.getUnassignedItems(), (p) => p.isDuplicate() && p.isPlayer() && !p.isLimitedUse()),
              "duplicateId"
            );
            let gf = {
              id: dupIds,
            ...
            if (up.length) {
              if (
                repositories.Item.getUnassignedItems().filter((i) => {
                  return i.duplicateId;
                }).length > up.length
              ) {
                events.notice("notice.dupfilldiff", 1);
              }
              const slotPlayer = b._parent.squad
                .getPlayers()
                .filter((slot) => slot.item.id !== 0)
                .map((slot) => slot.item);
        ...
        b._parent = e;
        this._fsuDupFill = b;
        exchangeElement.before(this._fsuDupFill.__root);
        this._fsuIgnore.show();
      }
      ...
      if (!this._fsuSquadCmpl && info.set.sbc_squadcmpl && targetRting) {
        ...
        exchangeElement.before(this._fsuSquadCmpl.__root);
        this._fsuIgnore.show();
        events.sbcFilterTipsGenerate("_fsuIgnore", this, 2);
        events.sbcFilterTipsGenerate("_fsuIgnore", this, 4);
      }
      ...
      if (needChem) {
        ...
        //非需求默契状态下
        Object.assign(this._btnSquadBuilder.__root.style, {
          width: "calc(100% - 1rem)",
          marginTop: ".675rem",
          margin: ".5rem auto",
        });
        this._btnSquadBuilder.addClass("mini");
        rewardElement.appendChild(this._btnSquadBuilder.__root);

        Object.assign(this._fsuSquad.__root.style, {
          width: "calc(100% - 1rem)",
          marginTop: ".675rem",
          margin: ".5rem auto",
        });
        this._fsuSquad.addClass("mini");
        rewardElement.appendChild(this._fsuSquad.__root);
      }

      this._challengeDetails.__root.style.backgroundColor = "#222426";
      this.__root.querySelector(".sbc-button-container").style.paddingTop = "1rem";
    };
    ...
    events.sbcFilterTipsGenerate = (elementName, target, type, mode) => {
      ...
      if (elementName in target && writeMode) {
        ...
        if (type == 1) {
          ...
          const options = ["ignorepos", "untradeable", "league", "flag", "academy"],
            optionsResult = [];
    ...
    events.getTemplate = async function (e, type, sId) {
      e.setInteractionState(0);
      let squadPos = e.challenge.squad.getFieldPlayers().map((i) => {
        return i.isBrick() ? null : i.getGeneralPosition();
      });
      events.showLoader();
    ...
    events.saveSquad = async (c, s, l, a) => {
      ...
      s.removeAllItems();
      s.setPlayers(l, true);
      await services.SBC.saveChallenge(c).observe(this, async function (z, d) {
        if (!d.success) {
        ...
        services.SBC.loadChallengeData(c).observe(this, async function (z, { response: { squad } }) {
          events.hideLoader();
    ...
    UTSBCService.prototype.loadChallengeData = function (r) {
      var s = this,
        a = new EAObservable();
      return (
        this.sbcDAO.loadChallenge(r.id, r.isInProgress()).observe(this, function (t, e) {
          t.unobserve(s);
          a.notify(e);
        }),
        a
    ...
    events.getFutbinSbcSquad = async (id, type) => {
      ...
      let url =
        type == 1
          ? `https://www.futbin.org/futbin/api/getChallengeTopSquads?chal_id=${id}&platform=${platform}`
          : type == 2
          ? `https://www.futbin.org/futbin/api/getSquadByID?squadId=${id}&platform=${platform}`
        ...
        if ((repositories.Item.getPileSize(ItemPile.TRANSFER) - repositories.Item.numItemsInCache(ItemPile.TRANSFER) > 0 || t) && price) {
          ...
          if (i.hasPriceLimits()) {
            if (p < i._itemPriceLimits.minimum || p > i._itemPriceLimits.maximum) {
              events.notice(["notice.auctionlimits", i._staticData.name], 2);
              return;
            }
          }
    ...
    events.playerGetLimits = async (i) => {
      return new Promise((resolve) => {
        ...
        services.Item.requestMarketData(i).observe(this, async function (sender, response) {
          resolve();
        });
      });
    };
    ...
    events.saveOldSquad = (s, t, c) => {
      if (s.isSBC() && (!info.base.savesquad || !t)) {
        ...
        let pl = s.getPlayers().map((i) => {
          return i.getItem();
        });
        if (
          s._fsuOldSquadCount == -1 ||
          s._fsuOldSquad[s._fsuOldSquadCount]
            .map((i) => {
              return i.id;
            })
            .join() !==
    ...
    events.getRatingPlayers = (squad, ratings) => {
      ...
      const buildExclusionList = (players) => {
        return players.map((i) => (i.item.rating && !i.item.concept ? i.item.databaseId : null)).filter(Boolean);
      };
    ...
    UTStoreView.prototype.setPacks = function (e, t, i, o) {
      ...
      if (HideAndShow) {
        ...
        // 25.22 增加新包的new标识
        e.forEach((item) => {
          item.isNew = item.start && now - item.start <= ONE_DAY;
        });
      ...
      setTimeout(() => {
        ...
        if (packTileExists || SBCTileExists) {
          ...
          if (info.set.info_packagain) {
            let packTile = events.createTile(fy("douagain.packtile.title"), fy("douagain.packtile.text"), (e) => {
              let current = cntlr.current();
              let pack = current.viewmodel
                .getPacks("mypacks")
                .filter((i) => i.id == info.douagain.pack)
                .pop();
              current.eOpenPack(current.getView(), UTStorePackDetailsView.Event.OPEN, { articleId: pack.id, tradable: pack.tradable });
            });
          ...
          if (info.set.info_sbcagain) {
            let sbcTile = events.createTile(fy("douagain.sbctile.title"), fy("douagain.sbctile.text"), (e) => {
        ...
        if (!unassignedTile && unassignedItems) {
          ...
          this._fsuUnassignedTile.addTarget(
            ...
            (e) => {
              TelemetryManager.trackEvent(TelemetryManager.Sections.STORE, TelemetryManager.Categories.BUTTON_PRESS, "Store - Unassigned Tile"), cntlr.current().gotoUnassigned();
            },
            EventType.TAP
          );
          itemListElement.insertBefore(tileBox, itemListElement.firstChild);
    ...
    events.needRatingsCount = (target, squad) => {
      ...
      if (squad) {
        ratings = _.map(
          _.filter(squad.getFieldPlayers(), (i) => {
            return i.item.isValid();
          }),
          "item.rating"
        );
        ...
        if ((e.unobserve(controller), t.success && 0 < t.data.challenges.length)) {
          if (SBCSetEntity.hidden) {
              ...
              services.SBC.loadChallenge(r).observe(controller, (ee, tt) => {
                if ((ee.unobserve(controller), tt.success)) {
                  ...
                  if (i) {
                    ...
                    o.initWithSBCSet(SBCSetEntity, r.id), i.pushViewController(o);
                  }
                } else {
            ...
            if (i) {
              ...
              o.initWithSBCSet(SBCSetEntity), i.pushViewController(o, !0);
          ...
          s.setInteractionState(!0), services.Notification.queue([services.Localization.localize(n), UINotificationType.NEGATIVE]);
        }
      });
    };
    events.setPackTileText = (packTile) => {
    ...
    events.judgmentSbcCount = (SBCTile) => {
      if (services.SBC.repository.getSets().length) {
        ...
        services.SBC.requestSets().observe(this, (e, t) => {
          if ((e.unobserve(this), t.success)) {
            events.setSbcTileText(SBCTile);
          } else if (NetworkErrorManager.checkCriticalStatus(t.status)) NetworkErrorManager.handleStatus(t.status);
          else {
    ...
    UTPackAnimationViewController.prototype.runAnimation = function () {
      if (!this.running) {
        ...
        e.setPackTier(this.packTier), e.generateItem(this.presentedItem);
    ...
    events.isSBCCache = (id, cId) => {
      ...
      if (SBCSetEntity) {
        ...
        services.SBC.requestSets().observe(cntlr.current(), function (e, t) {
          if ((e.unobserve(cntlr.current()), t.success && JSUtils.isObject(t.data))) {
            events.fastSBC(id, cId);
          } else {
    ...
    events.fastSBC = async (id, cId) => {
      ...
      services.SBC.requestChallengesForSet(SBCSetEntity).observe(controller, (e, t) => {
        if ((e.unobserve(controller), t.success && 0 < t.data.challenges.length)) {
                ...
                if (oneFillNeed && Object.keys(oneFillNeed).length) {
                  ...
                  if (!info.build.strictlypcik && events.isEligibleForOneFill(oneFillNeed)) {
                    ...
                    for (let i of oneFillNeed) {
                      ...
                      console.log(
                        _.map(searchResults, (i) => {
                          return i._staticData.name + `_` + i.rating;
                        })
                      );

                      excludeId = excludeId.concat(
                        searchResults.map((i) => {
                          return i.databaseId;
                        })
                      );
                            ...
                            if (0 < newChallenge.awards.length) {
                              ...
                              challengeRewards.init(),
                                ...
                                challengeRewards.onExit.observe(controller, function (e) {
                                  e.unobserve(controller), events.showRewardsView(SBCSetEntity);
                                });
                            } else {
          ...
          l.setInteractionState(!0), services.Notification.queue([services.Localization.localize(a), UINotificationType.NEGATIVE]);
          events.hideLoader();
        }
      });
      view.setInteractionState(!0);
    ...
    events.showRewardsView = (set) => {
      ...
      rewardsController.init(), (rewardsController.modalDisplayDimensions.width = "24em");
    ...
    UTSelectItemFromClubViewController.prototype.handleItemRetrieval = function (t, e) {
      ...
      if (this?.squad?.isSBC()) {
        if (!("_fsulistfilter" in this)) {
          let filterContainer = this.parentViewController.childViewControllers.find((item) => item.className === `UTClubSearchFiltersViewController`) || null;
          let isDream = false;
          if (filterContainer) {
          ...
          let fsuListFilter = {};
          let slotIds = _.map(
            _.filter(this.squad.getPlayers(), (slot) => {
              return slot.item.isPlayer() && slot.item.databaseId;
            }),
            "item.databaseId"
          );
          ...
          _.times(4, (i) => {
            ...
            if (filterBtn._type == 1 || filterBtn._type == 4) {
              filterBtn._text = [fy(`listfilter.text.asc`), fy(`listfilter.text.desc`)];
            }
            if (filterBtn._type == 2) {
              filterBtn._text = [fy(`listfilter.text.all`), fy(["listfilter.text.only", fy("listfilter.text.storage")])];
            }
            if (filterBtn._type == 5) {
              filterBtn._text = [fy(`listfilter.text.all`), fy(["listfilter.text.only", fy("listfilter.text.club")])];
            }
            if (filterBtn._type == 3) {
              filterBtn._text = [fy(`listfilter.text.all`), fy(["listfilter.text.only", listFilterPos ? listFilterPos.typeName : "ALL"])];
            }
    ...
    events.setListFilterTitleAndState = (element, players, initPlayers) => {
      ...
      if (!("_fsuAllChem" in parentElement)) {
        ...
        currentChem = _.map(players, (p) => {
          return parentElement._fsuAllChem[p.id];
        });
      }
      ...
      let tBtn = element[scopeKey],
        pBtn = element[3];
      let fp, afp;
      if (scopeKey == 2) {
        if (!("_fsuAllStorage" in parentElement)) {
          afp = _.map(
            _.filter(initPlayers, (p) => repositories.Item.storage.get(p.id)),
            "id"
          );
      ...
      if (!("_fsuPosPlayers" in parentElement)) {
        app = _.map(
          _.filter(initPlayers, (p) => _.includes(p.possiblePositions, pBtn._pos.typeId)),
          "id"
        );
    ...
    events.accelePopup = (player, isLoadMeta) => {
      ...
      services.PlayerMetaData.updateItemPlayerMeta([player]).observe(cntlr.current(), function (t, e) {
        ...
        _.forEach(document.querySelectorAll(`.fsu-cards-accele[data-defid="${player.definitionId}"]`), (el) => {
        ...
        _.forEach(acceleResults, (value, key) => {
          ...
          _.forEach(value, (i) => {
            resultsHtml += `<div class="item" style="display: flex; align-items: center;"><div class="playStyle chemstyle${i}" style="font-size: 18px; margin-right: 6px;"></div><div>${services.Localization.localize(
              `playstyles.playstyle${i}`
            )}</div></div>`;
          });
          resultsHtml += `</div>`;
        ...
        events.popup(fy("accelerate.popupt"), accelePopupText, (t) => {});
      ...
      if (services.Configuration.checkFeatureEnabled(UTServerSettingsRepository.KEY.STORAGE_PILE_ENABLED)) {
        ...
        services.Item.searchStorageItems(e).observe(v, function (e, t) {
          ...
          if ("_fsuStorageTile" in this) {
            ...
            this._fsuStorageTile.addTarget(
              this,
              (e) => {
      ...
      if ("_fsuLock" in this && this._fsuLock) {
        ...
        t.update(this.searchCriteria),
          ...
          services.Club.search(t).observe(this, function (e, t) {
            var i;
            if ((e.unobserve(s), s.clubViewModel && t.success && JSUtils.isObject(t.response))) {
              //console.log(t)
            ...
            } else services.Notification.queue([services.Localization.localize("notification.club.failedToLoad"), UINotificationType.NEGATIVE]), null === (i = s.getNavigationController()) || void 0 === i || i.popViewController(!0);
          });
    ...
    UTClubSearchResultsViewController.prototype.setupHeader = function (...args) {
      ...
      if ("_fsuStorage" in this && this._fsuStorage) {
        ...
        if (sendClubPlayers.length) {
          ...
          setClubHeader.init(), setClubHeader.hideActionButton(), setClubHeader.hideBulkActionButton(), setClubHeader.setText(fy([`storage.setclub.text`, sendClubPlayers.length]));
          let controller = this;
      ...
      if ("_fsuGoToEI" in this.getView()) {
        ...
        let t = this.getView().__ctaButtonContainer,
          b = events.createButton(
            ...
            (e) => {
              GM_openInTab(`https://www.easysbc.io/evolutions/players`, { active: true, insert: true, setParent: true });
            },
    ...
    UTStoreViewController.prototype.setCategory = function (e) {
      ...
      if (this.viewmodel !== void 0) {
        ...
        let searchCategoryIds = _.map(
          _.filter(this.viewmodel.categories, (obj) => conditions.includes(obj.localizedName)),
          "categoryId"
        );
    ...
    events.getPlayerMetaPopupText = (meta, pos) => {
      ...
      return fy(["plyers.relo.popupm", meta.name, sl.localize(`playstyles.playstyle${meta.chemstyle}`), desc, v, meta.rank, meta.rating]);
    };

    //24.23 增加读取模型属性
    events.getPlayerBodyType = (id, baseId, overall) => {
      if (overall >= 65) {
        return _.get(info.meta.bodyType, id) || _.get(info.meta.bodyType, baseId) || info.meta.baseBodyType;
      } else {
        return 0;
      }
    };
    ...
    events.jsonToItemEntity = (json, isUntradeable) => {
      ...
      const items = _.map(json.packItem.items, (i) =>
        _.assign({}, baseItem, {
          assetId: i.id,
          resourceId: i.id,
          rating: i.rating,
      ...
      if (items && items.length) {
        return _.map(items, (i) => {
          return itemFactory.createItem(i);
        });
      } else {
        return false;
      }
    ...
    events.tryPackPopup = async (pack, items) => {
      ...
      footInfo_2.insertAdjacentHTML(
        "beforeend",
        `<span class="currency-coins">${packOddo.toLocaleString()}</span><span style="padding-left: 1rem;">${fy("trypack.foot.info2_2")}</span><span class="currency-coins trypack-count">0</span><span style="padding-left: 1rem;">${fy(
          "trypack.foot.info2_3"
        )}</span><span class="trypack-diff">0%</span>`
      );
    ...
    events.autoBuySearchPlayer = (inputSelected, controller) => {
      ...
      criteria.sortBy = "ovr";
      services.Item.searchConceptItems(criteria).observe(controller, async (e, t) => {
        if ((e.unobserve(controller), JSUtils.isObject(t.response) && t.response.items)) {
          try {
            const PlayerName = inputSelected.name
              .normalize("NFKD")
              .replace(/[\u0300-\u036f]/g, "")
              .replace(/ø/g, "o");
    ...
    SBCCount.createElement = (ne) => {
      ...
      if (!_.has(info.nave, "SBCCount")) {
        info.nave.SBCCount = events.createButton(
          ...
          async (e) => {
            events.popup(fy("sbccount.popupt"), fy("sbccount.popupm"), (t) => {});
          },
    ...
    events.openPacksConfirmPopup = (packId, packName, packCount) => {
      let popupController = new EADialogViewController({
        dialogOptions: [{ labelEnum: enums.UIDialogOptions.OK }, { labelEnum: enums.UIDialogOptions.CANCEL }],
          ...
          if (!desc && i._playStyles.length) {
            ...
            _.map(_.orderBy(i._playStyles, [(item) => (item.isIcon ? 0 : 1), "category"], ["asc", "asc"]), (t) => {
              ...
              traitBox.appendChild(
                events.createElementWithConfig("i", {
                  classList: classList,
                })
              );
            });
    ...
    lock.init = function () {
      let a = JSON.parse(GM_getValue("lock_26", "[]")),
        b = [];
      if (a && typeof a === "object") {
        b = a;
      }
